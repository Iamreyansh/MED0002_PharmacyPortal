locals {
  github_subjects = {
    deploy = [
      "repo:${local.repo_node}:environment:staging",
      "repo:${local.repo_slug}:environment:staging",
    ]
    terraform_apply = [
    ...
    terraform_plan = [
      ...
      "repo:${local.repo_slug}:environment:terraform-plan-staging",
    ]
  }
  ...
  terraform_state = [
    ...
    },
    {
      Effect   = "Allow"
      Action   = ["s3:GetObject"]
      Resource = ["arn:aws:s3:::${var.tf_state_bucket}/MED0002/staging/*"]
    },
  ]
...
module "deploy_role" {
  ...
  name              = "${var.project_name}-staging-github-actions"
  description       = "Staging application deploy"
...
module "terraform_plan_role" {
  ...
  name              = "${var.project_name}-staging-github-terraform-plan"
  description       = "Staging terraform plan (read-only)"
  oidc_provider_arn = data.aws_iam_openid_connect_provider.github.arn
...
module "terraform_apply_role" {
  ...
  name              = "${var.project_name}-staging-github-terraform"
  description       = "Staging terraform apply"
  oidc_provider_arn = data.aws_iam_openid_connect_provider.github.arn
