...
import { getSessionSnapshot } from '@/modules/session/store/snapshot';
...
export function sessionFromLogin(
  ...
  return {
    ...
    session: {
      ...
      tokenScope,
      permissions:
        staff?.role === 'pharmacy_owner' || staff?.role === 'owner'
          ? ['*']
          : [],
      pharmacyName,
...
export function sessionFromMe(
  ...
  const pharmacyName = asString(active?.name) ?? previous.pharmacyName;
...
export function sessionFromPosPin(data: Record<string, unknown>): {
...
export function applyRegistrationStatus(
  ...
  return {
    ...
    pharmacyName: asString(data.business_name) ?? previous.pharmacyName,
